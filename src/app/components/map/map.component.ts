...
export class MapComponent implements AfterViewInit {
      ...
      map
        ...
        .addLayer({
          ...
          paint: {
            "circle-color": [
              "case",
              ["boolean", ["feature-state", "selected"], false], S4_ORANGE,
              ["boolean", ["feature-state", "hovered"], false], S4_TEAL_DARK,
              S4_BLUE
            ],
            "circle-radius": [
              "interpolate",
            ...
            "circle-stroke-color": [
              "case",
              ["boolean", ["feature-state", "selected"], false], S4_DARK_D, S4_DARK_L,
            ],
            "circle-stroke-width": [
              "interpolate",
              ["exponential", 1.1],
        ...
        .on("mouseleave", changingStations, (evt) => this.onHoverEnd(evt, map))
  ...
  private updateCardList = (map: mapboxgl.Map) => {
    if(map.getZoom() >= 8) {
      setTimeout(() => {
        let featureCollection = map.queryRenderedFeatures(undefined, {layers: [changingStations]});
        console.log(featureCollection)
        if(featureCollection.length > 0) {
          let features: InspectionSite[] = featureCollection.map((feature: any) => {
            return {id: feature.id,...feature.geometry, ...feature.properties } as InspectionSite;
          });
          if(this._state.selected.value !== undefined) {
              let selected = this._state.getSelectedSite(this._state.selected.value);
          ...
          features.forEach((feature, i) => {
            ...
            if(!exists) {
              if(from !== undefined && options !== undefined) {
                ...
                let distance = parseFloat(turf.distance(from, to, options).toFixed(2));
